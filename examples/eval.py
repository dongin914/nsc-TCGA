...
def evaluate_nsc_models(best_model, x_test, t_test, e_test, t_train, e_train, times_ddh):
    ...
    # Check if there are uncensored samples
    uncensored_samples = np.any(e_test == 1)
    
    if uncensored_samples:
        selection = (t_test < t_train.max()) | (e_test == 0)
    else:
        selection = slice(None)  # Include all samples if all are censored
